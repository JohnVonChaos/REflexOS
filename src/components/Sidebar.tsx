    ...
    return (
        <div className="border-b border-gray-700/50">
            <button onClick={() => setIsCollapsed(!isCollapsed)} className="w-full flex justify-between items-center p-2 text-sm font-semibold text-gray-300 hover:bg-gray-800">
                <span className="flex items-center gap-2">{title} {count !== undefined && `(${count})`}</span>
        ...
        <aside className="w-96 bg-gray-900/70 border-r border-gray-700/50 flex flex-col">
            <header className="p-3 border-b border-gray-700/50 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {onImportChats && (
                        <button
                            onClick={() => { const el = chatImportInputRef.current; if (el) el.click(); }}
                            title="Import Chat Logs"
            ...
            <input type="file" accept=".json,.txt" ref={chatImportInputRef} onChange={onImportChats} className="hidden chat-import-input" />
                    <input type="file" accept=".json,.txt" ref={(el) => { /* placeholder for chat import, parent passes handler */ }} className="hidden chat-import-input" />
            ...
            <div className="p-2 border-b border-gray-700/50 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={() => { const el = document.querySelector('.chat-import-input') as HTMLInputElement | null; if (el) el.click(); }} title="Import chat logs (line-delimited JSON)" className="w-full flex items-center justify-center gap-2 p-2 text-xs rounded-md bg-gray-800 hover:bg-gray-700 text-gray-300 transition-colors"><UploadIcon /> Import Chats</button>
            ...
            <div className="flex-1 overflow-y-auto">
                <CollapsibleSection 
                    ...
                    actions={
                        <button onClick={handleSpeakNarrative} title="Read narrative aloud" className="p-1 rounded-full text-gray-400 hover:text-cyan-400 hover:bg-gray-700/50">
                            <SpeakerIcon />
                        </button>
                    }
                >
