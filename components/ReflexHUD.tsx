...
interface ReflexHUDProps {
    isOpen: boolean;
    onClose: () => void;
    embedded?: boolean; // New prop for embedding in modal
}

export const ReflexHUD: React.FC<ReflexHUDProps> = ({ isOpen, onClose, embedded = false }) => {
    ...
    useEffect(() => {
        if (scrollRef.current && entries.length > prevEntryCountRef.current) {
    ...
    return (
        <div className={containerClasses}>
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                {/* Left Panel: Context Orbit (Diffs) */}
                <div className="w-64 border-r border-cyan-900 bg-gray-900/20 p-4 overflow-y-auto hidden md:block">
                    <h3 className="text-xs font-bold text-cyan-700 mb-4 border-b border-cyan-900 pb-1 flex items-center gap-2">
                        <ClockIcon className="w-3 h-3" />
                        CONTEXT_ORBIT
                    </h3>
                    <div className="space-y-4 text-xs">
                        {/* Placeholder for Context Diff Visualization */}
                        <div className="p-3 border border-dashed border-cyan-900/50 rounded text-gray-500 text-center">
                            Awaiting Sequence...
                        </div>
                    </div>
                </div>
