...
import type { SRGModule } from '../types';
...
export const ModuleManager: React.FC<ModuleManagerProps> = ({
  ...
  React.useEffect(() => {
    if (isOpen) {
      setModules(srgModuleService.listModules());
    }
  }, [isOpen, refreshKey]);

  const handleToggle = async (id: string) => {
    await srgModuleService.toggleModule(id);
    setRefreshKey(k => k + 1);
  };

  const handleWeightChange = async (id: string, weight: number) => {
    await srgModuleService.updateModuleWeight(id, weight);
    setRefreshKey(k => k + 1);
  };
