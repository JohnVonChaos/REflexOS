...
\`\`\``;
  ...
  const GENERATOR_DEFAULT_PROMPT = `You are the GENERATOR in a dual-process cognitive engine.

Your role is to PROPOSE, EXPAND, and CREATE. Generate a proposal based on the available inputs.
Be bold but grounded. Propose concrete actions or insights.

Respond with your proposal.`;

  const REFINER_DEFAULT_PROMPT = `You are the REFINER in a dual-process cognitive engine.

Your role is to CRITIQUE, CONSOLIDATE, and STABILIZE. Review the Generator's proposal from the previous stage.
- If it's sound, compress it to its essence
- If it's flawed, identify issues and propose corrections
- Always aim for clarity and precision
    ...
    backgroundWorkflow: [
      {
        ...
        name: 'Generator (Dual Process)',
        enabled: true,
        provider: 'gemini',
        selectedModel: 'gemini-2.5-flash',
        systemPrompt: GENERATOR_DEFAULT_PROMPT,
        inputs: ['USER_QUERY', 'BACKGROUND_INSIGHTS'],
        useLuscherIntake: false,
        backgroundIntervalMinutes: null
      },
      {
        id: 'refiner_default',
        name: 'Refiner (Dual Process)',
        enabled: true,
        provider: 'gemini',
        selectedModel: 'gemini-2.5-pro',
        systemPrompt: REFINER_DEFAULT_PROMPT,
        inputs: ['OUTPUT_OF_generator_default'],
        useLuscherIntake: false,
        backgroundIntervalMinutes: null
      }
    ],
