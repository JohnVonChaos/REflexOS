...
class RecallWeaverService {
  ...
  // NEW: Helper to calculate gradient weight based on distance from center
  private calculateGradientWeight(distanceFromCenter: number, windowSize: number): number {
      if (distanceFromCenter > windowSize) return 0; // Outside window
      // Simple linear decay. Can be made more sophisticated (e.g., Gaussian)
      return Math.max(0, 1 - (distanceFromCenter / windowSize));
  }
